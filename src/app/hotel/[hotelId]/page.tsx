...
export default function HotelMenuPage() {
  ...
  const [hotel, setHotel] = useState<Awaited<
    ReturnType<typeof getHotel>
  > | null>(null);
  useEffect(() => {
    if (!hotelId) return;
    getProductsForHotel(hotelId as string)
      .then(setProducts)
  ...
  async function handleAddToCart(id: number) {
    ...
    try {
      await addItemsToCart(userId, accessToken, [
      ...
      <div className="flex items-center gap-6 mb-6 bg-white p-4 rounded-lg shadow-sm">
        ...
        <div className="flex-1">
          ...
          <div className="mt-2 text-xs text-slate-500">
            Staff members: {hotel?.workersCount ?? "—"}
          </div>
        </div>
      </div>
      {/* <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {products.map((p) => (
          <ProductCard
            ...
            isAdding={addingIds.includes(p.id)}
          />
        ))}
      </div> */}
    </div>
